...
为了解决tcp粘包的问题（tcp返回回来的包可能是当前的数据和下一条数据混在一起的），我们需要记录参数的长度 args_size
                    header_str中包含  service_name method_name args_size
16UserServiceLoginzhang san123456
16表示前16个字符是服务名+方法名

header_size(4个字节) + header_str + args_str
我们如果直接把header_size存成字符串那么我们读4个字节也又可能出现错误：比如"10000"读四个字节将出错
因此我们应该直接存储整型，整形不会超过4个字节
*/
...
void RpcProvider::SendRpcResponse(const muduo::net::TcpConnectionPtr &conn, google::protobuf::Message *response)
{
    ...
    conn->shutdown(); // 模拟http的短链接服务，用rpcprovider主动断开连接
}